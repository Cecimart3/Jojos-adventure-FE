...
class StoryPage extends Component {
  ...
  componentDidMount() {
    ...
    storyId
      ? storyCalls.nextStory(storyId).then((res) => {
          this.setState({
            storyLine: res.data,
            isDead: res.data.isDead,
  ...
  optionsLink = () => {
    ...
    return options.map((option) => (
      <Link
        ...
        className="story-container__option"
        key={option.toNextId}
        to={`/story/${option.toNextId}`}
      >
        {option.description}
      </Link>
    ));
  ...
  render() {
    ...
    const storylineHide = animationRun
      ? "story-container__storyline--hide"
      : "";
    ...
    return !storyLine ? (
      ...
      <main className="story-container">
        ...
        <Animation
          image={this.state.animationImage}
          animate={this.state.animationRun}
        ...
        <div className={`story-container__storyline  ${storylineHide}`}>
          <div className="story-container__text-container">
            ...
            {!isEnd && (
              <p className="story-container__text story-container__question">
                What should Jojo do next?
              </p>
            )}
          </div>
          <div className="story-container__buttons">{this.optionsLink()}</div>
        </div>
      </main>
    );
  }
